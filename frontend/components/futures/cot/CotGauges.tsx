...
export function CotGauges({ data }: { data: ProcessedCotRow[] }) {
  ...
  const mkRow = (label: string, cat: string, field: string, isSpread?: boolean): GRData => {
    const vals = hist52.map(d => ((d[market] as unknown as Record<string, number>)?.[field] ?? 0) as number);
    const min = Math.min(...vals), max = Math.max(...vals);
    const cv = (curr[market] as unknown as Record<string, number>)?.[field] ?? 0;
    const pv = (prev?.[market] as unknown as Record<string, number> | undefined)?.[field] ?? cv;
