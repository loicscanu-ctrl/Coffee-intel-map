...
export function CotHeatmap({ data }: { data: ProcessedCotRow[] }) {
  ...
  const gv = (d: ProcessedCotRow, field: string) => ((d[market] as unknown as Record<string, number>)?.[field] ?? 0) as number;

  const lsRows = lsFields.map(f => ({
    label: f.label,
    color: HM_CAT_COLORS[f.label] ?? "#64748b",
    vals: weeks13.map((d: ProcessedCotRow) => {
  ...
  const spreadRows = spreadFields.map(f => ({
    ...
    vals: weeks13.map((d: ProcessedCotRow) => gv(d, f.key)),
  }));
    ...
    <>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 overflow-x-auto">
        <div style={{ display: "grid", gridTemplateColumns: `72px repeat(${weeks13.length}, 1fr)`, gap: 2, marginBottom: 2 }}>
          <div />
          {weeks13.map((d: ProcessedCotRow, i: number) => (
