...
import { shiftMonth, fmtBags, shortMonthLabel, TT_STYLE, DAILY_COLORS } from "./brazilTypes";
...
function DailyRegChart({
  ...
  const calMo = currentMonth.slice(5);
      ...
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(v: unknown, name: unknown) => [v !== null ? fmtBags(Number(v)) : "—", String(name ?? "")]}
            labelFormatter={(l: unknown) => `Day ${l}`}
          />
...
export function DailyRegistrationSection() {
  ...
  useEffect(() => {
    fetch("/data/cecafe_daily.json")
      ...
      .then(setData)
      .catch((err) => console.error("[BrazilTab] cecafe_daily fetch failed:", err));
  }, []);
