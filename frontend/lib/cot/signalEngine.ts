...
export function evaluateSignals(rows: ProcessedCotRow[]): Signal[] {
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (pd === "up"   && pr === "up")
      add({ id:"CP1", name:"Normal Hedging",       category:"CP", categoryLabel:"Producer", market:mkt, severity:"info",
        text:"Producers locking in levels into price strength — standard hedging flow." });

    if (pd === "up"   && pr === "down")
      add({ id:"CP2", name:"Forced Liquidation",   category:"CP", categoryLabel:"Producer", market:mkt, severity:"warn",
        text:"Producers selling into weakness — suggests stock overhang or cash flow pressure regardless of price." });

    if (pd === "down" && pr === "up")
      add({ id:"CP3", name:"Bullish De-hedging",   category:"CP", categoryLabel:"Producer", market:mkt, severity:"warn",
        text:"Producers lifting hedges into rising price — signals expectation of further upside." });

    if (pd === "down" && pr === "down")
      add({ id:"CP4", name:"Defensive De-hedging", category:"CP", categoryLabel:"Producer", market:mkt, severity:"info",
        text:"Producers covering shorts on weakness — normal deleveraging, low conviction." });

    if (isHigh(ps, i))
      add({ id:"CP5", name:"Producer Exhaustion",  category:"CP", categoryLabel:"Producer", market:mkt, severity:"warn",
        text:"Producers near fully hedged (>75th pct, 52-week) — limited additional selling capacity from this actor." });

    if (isLow(ps, i))
      add({ id:"CP6", name:"Producer Dry Powder",  category:"CP", categoryLabel:"Producer", market:mkt, severity:"alert",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (rd === "up"   && pr === "down")
      add({ id:"CR1", name:"Normal Coverage",     category:"CR", categoryLabel:"Roaster", market:mkt, severity:"info",
        text:"Roasters adding coverage into price weakness — standard buying flow." });

    if (rd === "up"   && pr === "up")
      add({ id:"CR2", name:"Forced Coverage",     category:"CR", categoryLabel:"Roaster", market:mkt, severity:"warn",
        text:"Roasters buying into rising price — suggests coverage urgency, potentially being squeezed." });

    if (rd === "down" && pr === "down")
      add({ id:"CR3", name:"Coverage Reduction",  category:"CR", categoryLabel:"Roaster", market:mkt, severity:"info",
        text:"Roasters reducing coverage on weakness — expecting further downside or reducing exposure." });

    if (rd === "down" && pr === "up")
      add({ id:"CR4", name:"Unusual Liquidation", category:"CR", categoryLabel:"Roaster", market:mkt, severity:"warn",
        text:"Roasters reducing coverage into rising price — may signal demand destruction or blend substitution toward Robusta." });

    if (isLow(rs, i) && pr === "up")
      add({ id:"CR5", name:"Squeeze Risk",        category:"CR", categoryLabel:"Roaster", market:mkt, severity:"alert",
    ...
    if (isHigh(rs, i))
      add({ id:"CR6", name:"Roaster Dry Powder",  category:"CR", categoryLabel:"Roaster", market:mkt, severity:"warn",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (pd === "down" && rd === "up")
      add({ id:"CI1", name:"Commercial Convergence Bullish", category:"CI", categoryLabel:"Commercial", market:mkt, severity:"warn",
        text:"Both commercials aligned bullish — strong fundamental demand signal." });

    if (pd === "up"   && rd === "down")
      add({ id:"CI2", name:"Commercial Convergence Bearish", category:"CI", categoryLabel:"Commercial", market:mkt, severity:"warn",
        text:"Both commercials aligned bearish — strong fundamental supply pressure signal." });

    if (pd === "up"   && rd === "up")
      add({ id:"CI3", name:"Commercial Divergence",          category:"CI", categoryLabel:"Commercial", market:mkt, severity:"info",
        text:"Producers and roasters on opposite sides — normal hedging flow, market in balance." });

    if (pd === "flat" && rd === "flat")
      add({ id:"CI4", name:"Commercial Vacuum",              category:"CI", categoryLabel:"Commercial", market:mkt, severity:"alert",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (ml === "up" && pr === "up" && !isHigh(mls, i))
      add({ id:"ML1", name:"Fund Bullish Entry",      category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"info",
        text:"Funds building longs into price strength — conviction depends on magnitude of the move." });

    if (ml === "up" && pr === "up" && isHigh(mls, i))
      add({ id:"ML2", name:"Fund Bullish Exhaustion", category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"warn",
        text:"Funds adding longs but near capacity (>75th pct) — bullish momentum likely limited, reversal risk increasing." });

    if (ml === "up" && pr === "down")
      add({ id:"ML3", name:"Contrarian Fund Buying",  category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"warn",
    ...
    if (ml === "down" && pr === "down")
      add({ id:"ML4", name:"Fund Long Liquidation",   category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"info",
        text:"Funds reducing longs into falling price — bearish momentum, trend following." });

    if (ml === "down" && pr === "up")
      add({ id:"ML5", name:"Fund Long Exit",          category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"warn",
    ...
    if (isHigh(mls, i) && pr !== "up")
      add({ id:"ML6", name:"Fund Long Overhang",      category:"ML", categoryLabel:"MM Longs", market:mkt, severity:"alert",
        text:"Large speculative long position (>75th pct) with no price follow-through — significant liquidation risk." });

    // MS4 cross-reference inline (surfaced under ML for completeness)
    void ms; // used in MS block below
  }
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (ms === "up" && pr === "down" && !isHigh(mss, i))
      add({ id:"MS1", name:"Fund Bearish Entry",       category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"info",
        text:"Funds building shorts into price weakness — conviction depends on magnitude of the move." });

    if (ms === "up" && pr === "down" && isHigh(mss, i))
      add({ id:"MS2", name:"Fund Bearish Exhaustion",  category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"warn",
        text:"Funds adding shorts near capacity (>75th pct) — bearish momentum likely limited, short covering risk increasing." });

    if (ms === "up" && pr === "up")
      add({ id:"MS3", name:"Contrarian Fund Shorting", category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"warn",
        text:"Funds shorting into rising price — contrarian positioning, betting on reversal." });

    if (ms === "down" && pr === "up")
      add({ id:"MS4", name:"Fund Short Covering",      category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"info",
    ...
    if (ms === "down" && pr === "down")
      add({ id:"MS5", name:"Reluctant Short Cover",    category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"warn",
        text:"Funds covering shorts despite falling price — may signal exhaustion of bearish thesis." });

    if (isHigh(mss, i) && pr !== "down")
      add({ id:"MS6", name:"Fund Short Squeeze Risk",  category:"MS", categoryLabel:"MM Shorts", market:mkt, severity:"alert",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (ml === "up"   && ms === "down")
      add({ id:"MI1", name:"Speculative Conviction Bullish", category:"MI", categoryLabel:"MM Interaction", market:mkt, severity:"warn",
        text:"Funds adding longs and covering shorts simultaneously — strong bullish conviction." });

    if (ml === "down" && ms === "up")
      add({ id:"MI2", name:"Speculative Conviction Bearish", category:"MI", categoryLabel:"MM Interaction", market:mkt, severity:"warn",
        text:"Funds reducing longs and adding shorts simultaneously — strong bearish conviction." });

    if (ml === "up"   && ms === "up")
      add({ id:"MI3", name:"Speculative Confusion",          category:"MI", categoryLabel:"MM Interaction", market:mkt, severity:"warn",
        text:"Both sides growing — check net position to assess which side dominates. Cross-commodity check recommended." });

    if (ml === "down" && ms === "down")
      add({ id:"MI4", name:"Speculative Retreat",            category:"MI", categoryLabel:"MM Interaction", market:mkt, severity:"info",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (ml === "up"   && pd === "up"   && pr === "up")
      add({ id:"MPI1", name:"Classic Bullish Flow",              category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"info",
    ...
    if (ml === "up"   && pd === "up"   && pr === "down")
      add({ id:"MPI2", name:"Forced Market",                     category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"warn",
    ...
    if (ml === "up"   && pd === "down" && pr === "up")
      add({ id:"MPI3", name:"Squeeze Setup",                     category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"alert",
    ...
    if (ml === "down" && pd === "up"   && pr === "down")
      add({ id:"MPI4", name:"Bearish Capitulation",              category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"alert",
    ...
    if (ml === "down" && pd === "down" && pr === "up")
      add({ id:"MPI5", name:"Divergence Signal",                 category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"warn",
    ...
    if (isHigh(mls, i) && isLow(ps, i))
      add({ id:"MPI6", name:"Fund Overhang vs Producer Pressure",category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"alert",
    ...
    if (ml === "up"   && pd === "up"   && pr === "flat")
      add({ id:"MPI7", name:"Natural Market Balance",            category:"MPI", categoryLabel:"MM × Producer", market:mkt, severity:"info",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (ml === "up" && rd === "up" && pr === "up" && !isLow(rs, i))
      add({ id:"MRI1", name:"Double Buying Pressure",       category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"warn",
    ...
    if (ml === "up" && rd === "up" && pr === "up" && isLow(rs, i))
      add({ id:"MRI2", name:"Roaster Squeeze Confirmed",    category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"alert",
    ...
    if (ml === "up" && rd === "down" && pr === "up")
      add({ id:"MRI3", name:"Contrarian Divergence",        category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"warn",
    ...
    if (ml === "down" && rd === "down" && pr === "down")
      add({ id:"MRI4", name:"Bearish Convergence",          category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"alert",
    ...
    if (ml === "up" && rd === "flat" && pr === "up")
      add({ id:"MRI5", name:"Fund Buying vs Roaster Absence",  category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"warn",
    ...
    if (ml === "down" && rd === "up" && pr === "down")
      add({ id:"MRI6", name:"Roaster Coverage Opportunity", category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"info",
    ...
    if ((ml === "up" || ml === "down") && rd === "flat" && pd === "flat")
      add({ id:"MRI7", name:"Full Commercial Vacuum",       category:"MRI", categoryLabel:"MM × Roaster", market:mkt, severity:"alert",
  ...
  // NY (KC arabica)
  if (strNY !== null) {
    const backNY = strNY > 0;
    const conNY  = strNY < 0;
    const nyRoastPct = pct52(nyRoastL, i);

    if (backNY)
      add({ id:"CS1", name:"Backwardation Incentive", category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"info",
        text:"KC curve in backwardation — front premium rewards long holders, supportive of bullish positioning. Verify that roll yield exceeds the risk-free rate to confirm the incentive is real." });

    if (conNY && mmLDir["NY"] === "up")
      add({ id:"CS3", name:"Contango Pressure",       category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"warn",
        text:"KC in contango while funds build longs — negative carry works against long holders. Conviction must be strong to justify the position." });

    if (conNY && roastDir["NY"] === "up")
      add({ id:"CS4", name:"Contango Relief",         category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"info",
        text:"KC in contango while roasters add coverage — forward prices cheaper than spot, incentivizes forward buying. Normal and sustainable." });

    if (backNY && pStrNY !== null && strNY > pStrNY && nyRoastPct <= 0.25)
      add({ id:"CS5", name:"Deepening Inversion",     category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"alert",
        text:"KC backwardation deepening while roasters are under-covered (<25th pct) — cost of forward coverage increasing week-on-week, amplifying squeeze risk. Cross-check against roll window." });

    if (backNY && pStrNY !== null && strNY < pStrNY)
      add({ id:"CS6", name:"Inversion Easing",        category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"warn",
        text:"KC backwardation losing strength — reduces incentive for longs to hold, may trigger gradual long liquidation. If easing coincides with roll window, may be technically driven rather than structural." });

    if (conNY && isLow(nyProdS, i))
      add({ id:"CS7", name:"Structural Contango",     category:"CS", categoryLabel:"Curve Structure", market:"NY", severity:"warn",
        text:"KC in contango while producers are significantly under-hedged — forward prices above spot give producers attractive levels to add hedges. Potential selling overhang building." });
  }

  // LDN (RC robusta)
  if (strLDN !== null) {
    const backLDN = strLDN > 0;
    const conLDN  = strLDN < 0;
    const ldnRoastPct = pct52(ldnRoastL, i);

    if (backLDN)
      add({ id:"CS1", name:"Backwardation Incentive", category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"info",
        text:"RC curve in backwardation — front premium rewards long holders, supportive of bullish positioning. Verify that roll yield exceeds the risk-free rate." });

    if (conLDN && mmLDir["LDN"] === "up")
      add({ id:"CS3", name:"Contango Pressure",       category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"warn",
        text:"RC in contango while funds build longs — negative carry works against long holders. Conviction must be strong to justify the position." });

    if (conLDN && roastDir["LDN"] === "up")
      add({ id:"CS4", name:"Contango Relief",         category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"info",
        text:"RC in contango while roasters add coverage — forward prices cheaper than spot, incentivizes forward buying." });

    if (backLDN && pStrLDN !== null && strLDN > pStrLDN && ldnRoastPct <= 0.25)
      add({ id:"CS5", name:"Deepening Inversion",     category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"alert",
        text:"RC backwardation deepening while roasters are under-covered (<25th pct) — amplifying squeeze risk. Cross-check against roll window." });

    if (backLDN && pStrLDN !== null && strLDN < pStrLDN)
      add({ id:"CS6", name:"Inversion Easing",        category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"warn",
        text:"RC backwardation losing strength — reduces incentive for longs to hold, may trigger gradual long liquidation. Check if coincides with roll window." });

    if (conLDN && isLow(ldnProdS, i))
      add({ id:"CS7", name:"Structural Contango",     category:"CS", categoryLabel:"Curve Structure", market:"LDN", severity:"warn",
        text:"RC in contango while producers are significantly under-hedged — attractive levels for producers to add hedges. Potential selling overhang building." });
  }
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (obos > 80 && !isLow(rs, i))
      add({ id:"OB1", name:"Overbought Warning",       category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"warn",
    ...
    if (obos > 80 && isLow(rs, i))
      add({ id:"OB2", name:"Overbought but Supported", category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"warn",
    ...
    if (obos < 20 && isLow(mls, i))
      add({ id:"OB3", name:"Oversold Opportunity",     category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"warn",
    ...
    if (obos < 20 && isLow(ps, i))
      add({ id:"OB4", name:"Oversold but Vulnerable",  category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"warn",
    ...
    if (obos >= 20 && obos <= 80)
      add({ id:"OB5", name:"Neutral Zone",             category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"info",
    ...
    if (obos > 80 && pr === "down")
      add({ id:"OB6", name:"Divergence Warning",       category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"alert",
    ...
    if (obos < 20 && pr === "up")
      add({ id:"OB7", name:"Oversold Divergence",      category:"OB", categoryLabel:"OB/OS", market:mkt, severity:"warn",
  ...
  for (const mkt of ["NY", "LDN"] as const) {
    ...
    if (spDir === "up" && pr !== "up")
      add({ id:"SP1", name:"Spreading Increase",       category:"SP", categoryLabel:"Spreading", market:mkt, severity:"info",
    ...
    if (spDir === "down")
      add({ id:"SP2", name:"Spreading Decrease",       category:"SP", categoryLabel:"Spreading", market:mkt, severity:"info",
    ...
    if (spDir === "up" && str !== null && pStr !== null && str > 0 && str > pStr)
      add({ id:"SP3", name:"Spreading vs Inversion",   category:"SP", categoryLabel:"Spreading", market:mkt, severity:"warn",
